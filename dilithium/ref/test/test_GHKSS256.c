...
void benchmark()
{
    ...
    for (int i = 0; i < num_timings; i++)
    {
        printf("as_sign_1\n");
        start = get_cycles();
        as_sign_1_old(pks->A, pke->A, pke->B, ctx_r[0]);
        end = get_cycles();
    ...
    for (int i = 0; i < num_timings; i++)
    {
        printf("as_sign_2\n");
        start = get_cycles();
        as_sign_2_old(&sig, *pks, ctx_r, ctx_s, ctx_z, Sei, mu, dsi[0], w_i, h_wi, user, users);
        end = get_cycles();
...
void benchmark_dkg2()
{
    ...
    poly(*Share_S)[M] = malloc(sizeof(poly[LHAT][M])); // [LHAT][M]
    poly(*Share_E)[M] = malloc(sizeof(poly[KHAT][M])); // [KHAT][M]
    poly(*Si)[M] = malloc(sizeof(poly[LHAT][M]));
    poly(*Ei)[M] = malloc(sizeof(poly[KHAT][M]));
    POLY_2D_INIT(Share_S, LHAT, M);
    POLY_2D_INIT(Share_E, KHAT, M);
    POLY_2D_INIT(Si, LHAT, M);
    POLY_2D_INIT(Ei, KHAT, M);
    ...
    for (int i = 0; i < KHAT; i++)
    {
        for (int j = 0; j < M; j++)
        {
            binary_sampler(Ei[i][j].coeffs, N);
            poly_reduce(&Ei[i][j]);
            poly_ntt(&Ei[i][j]);
        }
    }
    ...
    printf("gen E\n");
    // dk_gen_2_compute_share(Si, Ei, Share_S, Share_E, users, 0);
    end = get_cycles();
    print_timing(start, end, "gen_sharing_polys");

    printf("Start compute share\n");
    start = get_cycles();
    compute_share(Share_S, P_coeff_S, users[0]);
    compute_share(Share_E, P_coeff_S, users[0]);
    end = get_cycles();
    print_timing(start, end, "compute shares");
    ...
    printf("Rest of setup\n");
    start = get_cycles();
    keygen_e_1d(pke->A, pke->B, pke->S);
    end = get_cycles();
    print_timing(start, end, "rest_of_setup");


    start = get_cycles();
    poly(*Bij)[M] = malloc(sizeof(poly[KHAT][M]));      // [USERS][KHAT][M]
    POLY_2D_INIT(Bij, KHAT, M);
    dk_gen_2_with_share_1user(
        Si,
        Ei,
        pke,
        Share_S,
    ...
    print_timing(start, end, "agg, rest of 2");

    POLY_2D_CLEAR(Share_S, LHAT, M);
    POLY_2D_CLEAR(Share_E, KHAT, M);
    POLY_3D_CLEAR(P_coeff_S, THRESHOLD, LHAT, M);
    // POLY_3D_CLEAR(P_coeff_E, THRESHOLD, KHAT, M);
}
...
void benchmark_dkg3()
{
    ...
    poly dummy;
    poly_init(&dummy);
    poly(*Bj)[KHAT][M] = malloc(sizeof(poly[KHAT][M]));
    POLY_2D_INIT(Bj, KHAT, M);

    // Allocate for 1 user, rest we get from dummy polys
    poly(*Sij)[LHAT][M] = malloc(sizeof(poly[LHAT][M]));
    POLY_2D_INIT(Sij, LHAT, M);

    poly_uniform(&dummy, seed, nonce++);
    ...
    for (int j = 0; j < KHAT; j++)
    {
        for (int k = 0; k < M; k++)
        {
            poly_uniform(&Bj[0][j][k], seed, nonce++);
            poly_ntt(&Bj[0][j][k]);
            poly_reduce(&Bj[0][j][k]);
            poly_uniform(&Sij[0][j][k], seed, nonce++);
            poly_ntt(&Sij[0][j][k]);
            poly_reduce(&Sij[0][j][k]);
        }
    }
    poly(*Be)[M] = malloc(sizeof(poly[KHAT][M]));
    POLY_2D_INIT(Be, KHAT, M);
    ...
    poly_1d_clear((poly *)Sij, LHAT * M);
    free(Sij);
    poly_1d_clear((poly *)Bj, KHAT * M);
    free(Bj);
}
